...
func PublishList(c *gin.Context) {
	...
	//得到用户的信息
	author := model.User{}
	model.Db_write.Table("user").Where("id = ?", user_id).First(&author)
	user_follow_list := user_id + "/follow/list"
	...
	for i := 0; i < len(video_sql); i++ {
		...
		res := model.Rdb.SIsMember(model.Ctx, strconv.Itoa(author.Id), strconv.Itoa(int(video[i].Id)))
		isFa, _ := res.Result()
		if isFa {
